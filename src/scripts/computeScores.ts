...
import { aggregateRestaurant, normalizeScores, type RestaurantScore, type NormalizedScore } from '../scoring/engine';
...
Scoring:
  ...
  if (idArgs.length > 0) {
    ...
    restaurantIds = idArgs[0].split(',').map(id => parseInt(id.trim())).filter(id => !isNaN(id));
  ...
  if (!normalizedOnly) {
    ...
    console.log(`✓ Computed and saved raw scores for ${scores.length} restaurants`);
    console.log();
  } else {
  ...
  if (!rawOnly) {
    ...
    console.log(`✓ Computed and saved normalized scores for ${normalizedScores.length} restaurants`);
    console.log();
  }
  ...
  const displayCount = Math.min(20, normalizedScores.length > 0 ? normalizedScores.length : scores.length);
  ...
  for (let i = 0; i < displayCount; i++) {
    ...
    if (normalizedScore !== undefined) {
      console.log(`   Normalized Score: ${normalizedScore.toFixed(3)} | Raw Score: ${rawScore.toFixed(2)}`);
    } else {
      console.log(`   Raw Score: ${rawScore.toFixed(2)}`);
    }
    console.log(`   Location: ${restaurant.address ?? 'N/A'}${restaurant.city ? `, ${restaurant.city}` : ''}`);
    console.log(`   Items: ${itemCount} | Threads: ${threadCount} | Weight: ${totalWeight.toFixed(2)}`);
    console.log();
  }
  ...
  const bottomCount = Math.min(20, normalizedScores.length > 0 ? normalizedScores.length : scores.length);
  const bottomItems = normalizedScores.length > 0
  ...
  if (normalizedScores.length > 0) {
    ...
    const rawMean = allRawScores.reduce((sum, s) => sum + s, 0) / allRawScores.length;
    ...
    const normMean = allNormalizedScores.reduce((sum, s) => sum + s, 0) / allNormalizedScores.length;
    const sortedNormScores = [...allNormalizedScores].sort((a, b) => a - b);
    const normMedian = sortedNormScores[Math.floor(sortedNormScores.length / 2)];
