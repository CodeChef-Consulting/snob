...
async function addRestaurantManually() {
  try {
    ...
    if (!restaurantName) {
      ...
      console.log('Example: tsx src/scripts/addRestaurantManually.ts "Guelaguetza Restaurant"');
      process.exit(1);
    }
    ...
    if (!place) {
      console.error(`❌ Restaurant "${restaurantName}" not found in Google Places`);
      process.exit(1);
    }
    ...
    if (existing) {
      ...
      console.log(`   Address: ${existing.address}, ${existing.city}, ${existing.state} ${existing.zipCode}`);
